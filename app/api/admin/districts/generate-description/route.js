...
export async function POST(request) {
    try {
        ...
        const { districtName, cityName, districtSlug } = await request.json();

        if (!districtName || !cityName) {
            return NextResponse.json({ error: 'İlçe adı ve şehir adı gerekli' }, { status: 400 });
        }
