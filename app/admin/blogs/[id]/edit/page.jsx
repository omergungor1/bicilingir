...
export default function EditBlog() {
    ...
    const fetchBlog = async () => {
        try {
            ...
            if (response.ok) {
                ...
                if (settings) {
                    setOptions({
                        province_id: blog.province_id,
                        district_id: blog.district_id,
                        neighborhood_id: blog.neighborhood_id,
                        service_id: blog.service_id,
                        locksmith_id: blog.locksmith_id,
                        ...
                        content_style: settings.content_style || 'normal',
                        include_list: settings.include_list || false,
                        include_table: settings.include_table || false,
                        include_faq: settings.include_faq || false,
                        include_internal_links: settings.include_internal_links || false,
                        include_cta: settings.include_cta || false,
                        include_year: settings.include_year || false,
                        light_humor: settings.light_humor || false
                    })
                }

                if (blog.blog_images) {
                    setSelectedImage(blog.blog_images)
                }
            } else {
    ...
    const handleSave = async (status) => {
        ...
        try {
            // Blog güncelle
            const blogResponse = await fetch(`/api/admin/blogs/${params.id}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    ...formData,
            ...
            if (blogResponse.ok) {
                // Blog ayarlarını güncelle
                await fetch(`/api/admin/blogs/${params.id}/settings`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-1 space-y-6">
                    ...
                    <div className="bg-white p-6 rounded-lg shadow">
                        <h3 className="text-lg font-medium text-gray-900 mb-4">Blog Durumu</h3>
                        <div className="space-y-2">
                            <p className="text-sm text-gray-600">
                                Mevcut Durum: <span className="font-medium capitalize">{formData.status}</span>
                            </p>
                            <select
                    ...
                    <div className="bg-white p-6 rounded-lg shadow">
                        ...
                        {aiSettingsExpanded && (
                            <div className="space-y-6">
                                ...
                                >
                                    {generating ? '🤖 AI Yeniden Üretiyor...' : '🤖 Yeniden Üret'}
                                </button>
                            </div>
                        )}
                    </div>
                ...
                {/* Sağ Panel - Form (Aynı new sayfasındaki gibi) */}
                <div className="lg:col-span-2 space-y-6">
                    ...
                    {/* SEO Alanları - Aynı new sayfasındaki gibi */}
                    <div className="bg-white p-6 rounded-lg shadow space-y-4">
