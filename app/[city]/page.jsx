...
async function getCityData(citySlug) {
    try {
        ...
        const sssList = [
            ...
            { id: 15, question: `${cityData.name}\'${getBulunmaEki(cityData.name)} kapı kilidi kırıldı, ne yapmalıyım?`, answer: `Kapı kilidiniz kırıldıysa, öncelikle güvenliğiniz için geçici önlemler alın ve hemen BiÇilingir üzerinden bir çilingir ile iletişime geçin. Çilingirlerimiz kilit değişimi ve tamir hizmetleri de sunmaktadır. Size uygun yeni kilit modelleri ve güvenlik önerileri konusunda da yardımcı olacaklardır.` },
            ...
            { id: 26, question: `${cityData.name} gece çilingir hizmeti güvenilir mi?`, answer: `Evet, BiÇilingir platformunda listelenen gece çilingir hizmetleri güvenilirdir. Tüm çilingirlerimiz kimlik kontrolü yapılarak platformumuza kabul edilmektedir. Gece hizmeti veren çilingirlerimiz de aynı profesyonel standartlarda çalışmaktadır. Müşteri değerlendirmeleri ve derecelendirmeleri inceleyerek en güvenilir çilingirleri seçebilirsiniz.` },
            ...
            sssList.push(
                {
                    ...
                    question: `${district.name}\'${getBulunmaEki(district.name)} çilingir fiyatları ne kadar?`,
                    answer: `${district.name}\'${getBulunmaEki(district.name)} çilingir fiyatları genellikle 300₺ ile 1000₺ arasında değişmektedir. Standart kapı açma işlemi 300₺-500₺, çelik kapı açma 500₺-800₺, kilit değişimi 400₺-1000₺ arasındadır. ${cityData.name} merkeze olan mesafeye bağlı olarak ek ücret talep edilebilir. En güncel fiyat bilgisi için BiÇilingir platformundaki çilingirlerle iletişime geçebilirsiniz.`
                },
                {
                    id: districtQuestionId++,
        ...
        sssList.push(
            ...
            {
                id: districtQuestionId++,
                question: `${cityData.name}\'${getBulunmaEki(cityData.name)} çilingirler fatura veriyor mu?`,
                answer: `Evet, ${cityData.name}\'${getBulunmaEki(cityData.name)} hizmet veren çilingirlerin çoğu talep edilmesi durumunda fatura düzenleyebilmektedir. Kurumsal işletmeler için vergi faturası veya fiş gerekiyorsa, hizmet almadan önce çilingire bu talebinizi iletmeniz önerilir. BiÇilingir platformunda fatura veren çilingirleri ayrıca filtreleyebilirsiniz.`
            },
            {
                id: districtQuestionId++,
